...
export default function UsaDashboard({ initialSales = [] }: { initialSales?: Sale[] }) {
  ...
  const sellerProfitAllocationsFor = (sale: Sale) => {
    ...
    const assignedPercentage = Number(customer?.profitPercentage || 0);
    const allocations = [
      { sellerName: assignedSeller, amount: distributableProfit * (assignedPercentage / 100) },
      { sellerName: profitSellerName("RR"), amount: distributableProfit * 0.33 },
      { sellerName: profitSellerName("GM"), amount: distributableProfit * 0.33 },
    ];
  ...
  const saleDraftSummary = useMemo(() => {
    ...
    const sellerProfit = Math.max(0, profit) * ((Number(customer?.profitPercentage || 0) + 66) / 100);
    return { total, profit, sellerProfit };
  }, [form, partners, saleLineItems]);
